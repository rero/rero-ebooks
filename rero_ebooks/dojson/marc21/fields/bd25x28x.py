...
def country_of_producing_entity(self, key, value):
    ...
    return {
        ...
        "field_link_and_sequence_number": utils.force_list(value.get("8")),
        "linkage": value.get("6"),
        "country_of_producing_entity": utils.force_list(value.get("a")),
        "source": value.get("2"),
    }
...
def publication_distribution_imprint(self, key, value):
    ...
    indicator_map1 = {
        ...
        "_": "Not applicable/No information provided/Earliest available publisher",  # nopep8
    }
    field_map = {
    ...
    return {
        "__order__": tuple(order) if len(order) else None,
        "place_of_publication_distribution": utils.force_list(value.get("a")),
        "date_of_publication_distribution": utils.force_list(value.get("c")),
        "date_of_manufacture": utils.force_list(value.get("g")),
        "place_of_manufacture": utils.force_list(value.get("e")),
        "manufacturer": utils.force_list(value.get("f")),
        "field_link_and_sequence_number": utils.force_list(value.get("8")),
        "materials_specified": value.get("3"),
        "linkage": value.get("6"),
        "name_of_publisher_distributor": utils.force_list(value.get("b")),
...
def imprint_statement_for_films_pre_aacr_1_revised(self, key, value):
    ...
    return {
        "__order__": tuple(order) if len(order) else None,
        "producing_company": utils.force_list(value.get("a")),
        "place_of_production_release": utils.force_list(value.get("f")),
        "contractual_producer": utils.force_list(value.get("e")),
        "field_link_and_sequence_number": utils.force_list(value.get("8")),
        "linkage": value.get("6"),
        "releasing_company": utils.force_list(value.get("b")),
        "date_of_production_release": utils.force_list(value.get("d")),
    }
...
def production_publication_distribution_manufacture_and_copyright_notice(
    ...
    field_map = {
        ...
        "c": "date_of_production_publication_distribution_manufacture_or_copyright_notice",  # nopep8
        "8": "field_link_and_sequence_number",
        "3": "materials_specified",
        "6": "linkage",
    ...
    return {
        ...
        "date_of_production_publication_distribution_manufacture_or_copyright_notice": utils.force_list(  # nopep8
            value.get("c")
        ),
        "field_link_and_sequence_number": utils.force_list(value.get("8")),
        "materials_specified": value.get("3"),
...
def address(self, key, value):
    ...
    return {
        ...
        "telephone_number": utils.force_list(value.get("k")),
        "terms_preceding_attention_name": value.get("f"),
        "postal_code": value.get("e"),
        "fax_number": utils.force_list(value.get("l")),
        "linkage": value.get("6"),
        "city": value.get("b"),
        "country": value.get("d"),
        "specialized_telephone_number": utils.force_list(value.get("j")),
        "address": utils.force_list(value.get("a")),
        "contact_person": utils.force_list(value.get("p")),
        "public_note": utils.force_list(value.get("z")),
        "state_or_province": value.get("c"),
        "title_of_contact_person": utils.force_list(value.get("q")),
        "field_link_and_sequence_number": utils.force_list(value.get("8")),
        "relator_code": utils.force_list(value.get("4")),
        "hours": utils.force_list(value.get("r")),
        "tdd_or_tty_number": utils.force_list(value.get("n")),
